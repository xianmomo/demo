...
h3, h4, h5 {
	...
	<script type="text/javascript">
		$("#tbSvnApply_svnList").find("#dataGrid").datagrid({
			...
			queryParams:{
				applyStatu:"3",
				repositoryTypeNameIn:"('SVN系统库','SVN管理库')"
			},
			columns : [ [
					{
						title : '申请单Id',
		...
		//查询数据
		function seachData_tbSvnApply_svnList() {
			$("#tbSvnApply_svnList").find("#dataGrid").datagrid('load',{
				repositoryTypeNameIn:$("#tbSvnApply_svnList").find("#repositoryTypeNameIn").val(),
				applyStatu:$("#tbSvnApply_svnList").find("#applyStatu").val(),
				applyTimeBegin:$("#tbSvnApply_svnList").find("#applyTimeBegin").val(),
				applyTimeEnd:$("#tbSvnApply_svnList").find("#applyTimeEnd").val(),
				applyUserNameLike:$("#tbSvnApply_svnList").find("#applyUserNameLike").val(),
				processNumber: $("#tbSvnApply_svnList").find("#processNumber").val()
			});
		}

		//ECP审批
		function checkMenu_tbSvnApply_svnList() {
			//获取选中的行
			var selectRows = $("#tbSvnApply_svnList").find("#dataGrid").datagrid('getSelected');
			if (selectRows == null) {
		...
		//查看申请
		function showMenu_tbSvnApply_svnList() {
			//获取选中的行
			var selectRows = $("#tbSvnApply_svnList").find("#dataGrid").datagrid('getSelected');
			if (selectRows == null) {
		...
		function exceptionProcess_tbSvnApply_svnList(){
			//获取选中的行
			var selectRows = $("#tbSvnApply_svnList").find("#dataGrid").datagrid('getSelections');
			if (selectRows == null) {
	...
	<div id="tbSvnApply_svnList">
		<div style="padding: 5px 0;">
			<a onclick="checkMenu_tbSvnApply_svnList()" href='javascript:void("#")'
				class="easyui-linkbutton" data-options="iconCls:'icon-lock'">ECP审批</a>
			<a onclick="showMenu_tbSvnApply_svnList()" href='javascript:void("#")'
				class="easyui-linkbutton" data-options="iconCls:'icon-view'">查看申请</a>
			<a onclick="exceptionProcess_tbSvnApply_svnList()" href='javascript:void("#")'
		...
		<div style="padding: 5px 0;">
			<form class="custom-query-form">
				...
				<label-value>
					<select class="easyui-combobox" id="repositoryTypeNameIn">
						<option value="('SVN系统库','SVN管理库')">所有</option>
			...
			<label onclick="seachData_tbSvnApply_svnList();" class="easyui-linkbutton custom-search" data-options="iconCls:'icon-search'" style="width:80px">搜索</label>
		</div>
